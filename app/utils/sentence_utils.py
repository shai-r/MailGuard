...
def search_words_in_sentence(words: List[str]):
    return lambda sentence: any(word in sentence for word in words)

search_suspicious_words_in_sentence = search_words_in_sentence(suspicious_words)

def order_sentences(sentences_list: List[str]):
    suspicious_sentences = list(filter(
        search_suspicious_words_in_sentence, sentences_list
    ))
    not_suspicious_sentences = list(filter(
        lambda s: not search_suspicious_words_in_sentence(s), sentences_list
    ))
